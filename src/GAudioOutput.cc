...
#endif
{
    ...
    connect(this, &GAudioOutput::playSound, worker, &QGCAudioWorker::noticeTips);
...
void GAudioOutput::noticeTips(int noticeType, int severity)
{
    if (!muted)
    {
        emit playSound(noticeType,severity);
    }
}
...
bool GAudioOutput::saySyn(const QString& inText, int severity)
{
    if (!muted) {
#if defined __android__
#if defined QGC_SPEECH_ENABLED
...
#endif
#else
        emit textToSpeakSyn(inText, severity);
#endif
    }
    return true;
}
