    ...
    , _forceClose(false)
{
    ...
    /*add by xuzhitong*/
    connect(_ui.actionSpeech_control,  &QAction::triggered, this, &MainWindow::switchSpeechControlModeCallback);
...
/*add by xuzhitong*/
void MainWindow::switchSpeechControlModeCallback()
{
    bool speechControlMode = !(qgcApp()->_getSpeechControlMode());

     qDebug()<<"switchSpeechControlModeCallback "<<speechControlMode;
     qgcApp()->_setSpeechControlMode(speechControlMode);
     if (speechControlMode)
     {
         //if (!qgcApp()->runningUnitTests())
     ...
     {
         /*if (!qgcApp()->runningUnitTests())
             qgcApp()->toolbox()->audioOutput()->say(QString("Now you have switched back to the normal control mode."), GAudioOutput::AUDIO_SEVERITY_NOTICE);*/
     }
...
void MainWindow::configureWindowName()
{
    ...
    QString windowname = "杭州智鹍科技有限公司V1.0";
