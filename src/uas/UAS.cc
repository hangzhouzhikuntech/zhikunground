...
void UAS::controlExternalDevices(UASInterface::StartExternalDeviceType ExtType,qint32 * params)
{
    if (!_vehicle) {
        return;
    }

    int alarmLightFlag = 1;
    int cmdId = 0;
    float paramsArry[7] = {0};

    memset(paramsArry,0,sizeof(int)*7);
    if (params)
    {
        for (int i = 0;i<7;i++)
        {
    ...
    switch (ExtType)
    {
        case StartExternalDeviceOpenAlarmLight:
        {
            cmdId = 253;
            alarmLightFlag = 2;
            paramsArry[1] = 2;
        ...
        case StartExternalDeviceCloseAlarmLight:
        {
            cmdId = 253;
            alarmLightFlag = 1;
            paramsArry[1] = 1;
        ...
        case StartExternalDeviceGimbalStart:
        {
            qDebug()<<"start adjust Gimbal";
            paramsArry[6] = 2;
            paramsArry[5] = 1;
            cmdId = 205;
            break;
        }
        case StartExternalDeviceGimbalAdjust:
        {
            qDebug()<<"adjusting Gimbal";
            int data = paramsArry[0];
            int data_type = paramsArry[1];
            cmdId = 205;
            paramsArry[6] = 2;
            paramsArry[5] = data_type;
            switch (data_type)
            {
                case 0:
                {
        ...
        case StartExternalDeviceGimbalStop:
        {
            qDebug()<<"stop adjust Gimbal";
            cmdId = 205;
            paramsArry[6] = 2;
            paramsArry[5] = 0;
            break;
    ...
    qDebug()<<"controlExternalDevices send mavlink message cmdid:"<<cmdId<<",params[1]:"<<paramsArry[0]<<",params[2] :"<<paramsArry[1]<<",params[3]:"<<paramsArry[2];
    mavlink_message_t msg;
    mavlink_msg_command_long_pack(mavlink->getSystemId(),
                                  mavlink->getComponentId(),
                                  &msg,
                                  ...
                                  cmdId,    // command id
                                  0,                                // 0=first transmission of command
                                  paramsArry[0],                          // gyro cal
                                  paramsArry[1],                           // mag cal
                                  paramsArry[2],                                // ground pressure
                                  paramsArry[3],                         // radio cal
                                  paramsArry[4],                         // accel cal
                                  paramsArry[5],                      // airspeed cal
                                  paramsArry[6]);                          // esc cal
    _vehicle->sendMessage(msg);
}
