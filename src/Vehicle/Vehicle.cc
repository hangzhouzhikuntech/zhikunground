...
void Vehicle::_handleHomePosition(mavlink_message_t& message)
{
    ...
    //qDebug()<<"_handleHomePosition222222";
    ...
    if (emitHomePositionChanged) {
        ...
        //qDebug()<<"home position changed 11111";
...
void Vehicle::_handletextMessageReceived(UASMessage* message)
{
    if(message)
    {
        ...
        //qDebug()<<"recv uas message "<<message->getText();
...
void Vehicle::_connectionActive(void)
{
    ...
    if (_connectionLost) {
        ...
        _noticeTips(QGCAudioWorker::NOTICE_TYPE_CONNECTED,GAudioOutput::AUDIO_SEVERITY_NOTICE);
    }
}
...
void Vehicle::_noticeTips(int noticeType, int severity)
{
    if (!qgcApp()->runningUnitTests())
        qgcApp()->toolbox()->audioOutput()->noticeTips(noticeType, severity);
}
...
//add by xuzhitong
void Vehicle::_externalDeviceOperation(qint32 extType)
{
    qDebug()<<"_externalDeviceOperation type"<<extType;
    if (_uas)
    {

        _uas->controlExternalDevices((UASInterface::StartExternalDeviceType)extType);
    }
}
