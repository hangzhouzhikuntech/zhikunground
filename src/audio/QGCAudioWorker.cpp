...
#include <QSound>
static const QString soundFile[6] =
{
    "connected","disconnect","alarm","lowbattery","lowelectricity","leftelectricity"
};
...
void QGCAudioWorker::getSoundFromType(int noticeType,QString &soundStr)
{
    if (noticeType < NOTICE_TYPE_CONNECTED || noticeType > NOTICE_TYPE_LEFTELECT)
    {
        return;
    }
    soundStr = soundFile[noticeType];
}
...
void QGCAudioWorker::saySyn(QString inText, int severity)
{
    say(inText, severity);
