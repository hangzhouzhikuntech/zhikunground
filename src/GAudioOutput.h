...
public:
    ...
    bool isMuted();
    bool isPlaying();
    void noticeTips(int noticeType,int severity = 6);

public slots:
    /** @brief Say this text if current output priority matches */
    bool say(const QString& text, int severity = 6);
    //add by xuzhitong
    bool saySyn(const QString& text, int severity = 6);
...
signals:
    ...
    bool textToSpeakSyn(QString text, int severity = 1);
    void beepOnce();
    void playSound(int noticeType,int serverity=1);
